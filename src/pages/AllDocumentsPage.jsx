...
export default function AllDocumentsPage() {
  ...
  const docs = mockDocuments.filter((d) => {
    if (propertyFilter && d.property_eircode !== propertyFilter) return false;
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-xl font-bold" style={{ color: '#2C4459' }}>Documents</h1>
        ...
        <div className="flex gap-2">
          ...
          {properties.length > 1 && (
            ...
            >
              ...
              {properties.map((p) => (
                <option key={p.eircode} value={p.eircode}>{p.eircode}</option>
              ))}
            </select>
          )}
        </div>
      ...
      {docs.length === 0 ? (
        ...
        <div className="space-y-3">
          <p className="text-xs" style={{ color: '#9CA3AF' }}>{docs.length} documents</p>
          {docs.map((doc) => {
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1 flex-wrap">
                        <Badge
                          ...
                          text={ts.text}
                        />
                        <span className="text-[10px]" style={{ color: '#9CA3AF' }}>{doc.property_eircode}</span>
                      </div>
